function [ RHS, res0, eqn, opts, oper ] = init_res_default_gpu( eqn, opts, oper, RHS)
...
if isfield(opts, 'nm') && isfield(opts.nm, 'res0')
    ...
    if opts.LDL_T
        if opts.norm == 2
            res0 = max(abs(eig(mul_gpu(RHS',RHS) * diag(eqn.S_diag))));
        else
            res0 = norm(eig(mul_gpu(RHS',RHS) * diag(eqn.S_diag)), 'fro');
        end
    else
        res0 = norm(mul_gpu(RHS',RHS), opts.norm);
    end
end
end
